...
my_data = company_indexer.fit(my_data).transform(my_data)
...
data_new.write.option("header",True).format("parquet").save("final_project/new_data")

(train_set, test_set) = data_new.randomSplit([0.70, 0.30], seed = 42)

train_set.write.option("header",True).format("parquet").save("final_project/train_set")
test_set.write.option("header",True).format("parquet").save("final_project/test_set")
...
combined_train_set.write.option("header",True).format("parquet").save("final_project/combined_train_set")